...
import { useDocumentsToVerify } from '../../hooks/useDocuments';
import { supabase } from '../../lib/supabase';
import AuthenticatorStatsCards from '../AuthenticatorStatsCards';
...
export function DocumentManager({
  ...
  console.log('[DocumentManager] user:', user); // DEBUG: Ver papel do usuário
  ...
  const { documents: documentsToVerify, loading: loadingToVerify, error: errorToVerify, refetch: refetchToVerify } = useDocumentsToVerify();
  ...
  const handleCreateFolder = () => {
    ...
    const newFolder: Folder = {
      ...
      updated_at: new Date().toISOString(), // Corrigido
  ...
  // Aprovar documento: cria em translated_documents e remove de documents_to_verify
  async function handleApproveDocument(docToVerify: any) {
    if (!user) return;
    // Pega dados do documento original
    const docOrig = docToVerify.documents || {};
    const clienteId = docToVerify.user_id;
    const docId = docToVerify.doc_id;
    const filename = docOrig.filename || docToVerify.doc_url;
    const file_url = docOrig.file_url || docToVerify.doc_url;
    try {
      // 1. Cria em translated_documents
      ...
      // 2. Remove de documents_to_verify
      await supabase.from('documents_to_verify').delete().eq('id', docToVerify.id);
      // 3. Atualiza UI
      refetchToVerify();
    } catch (err) {
      let errorMsg = 'Erro ao aprovar documento';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Seção especial para autenticador */}
        {user?.role === 'authenticator' ? (
          <>
            {/* Cards de métricas para autenticador */}
            <AuthenticatorStatsCards documentsToVerify={documentsToVerify} />
            <div className="mb-8">
              <h2 className="text-xl font-bold mb-4 text-blue-800">Documentos para Verificar</h2>
              {loadingToVerify ? (
                <div className="text-gray-500">Carregando documentos...</div>
              ) : errorToVerify ? (
                <div className="text-red-500">Erro ao carregar documentos: {errorToVerify}</div>
              ) : documentsToVerify.length === 0 ? (
                <div className="text-gray-500">Nenhum documento para verificar.</div>
              ) : (
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {documentsToVerify.map((doc) => (
                        <tr key={doc.id}>
                          <td className="px-6 py-4 whitespace-nowrap">{doc.profiles?.name || doc.profiles?.email || 'Cliente'}</td>
                          <td className="px-6 py-4 whitespace-nowrap">{doc.documents?.filename || doc.doc_url}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <a href={doc.doc_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline mr-2">Baixar</a>
                            <button className="text-green-600 hover:underline mr-2" onClick={() => handleApproveDocument(doc)}>Aprovar</button>
                            <button className="text-yellow-600 hover:underline">Reenviar</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          // Dashboard padrão para clientes/admins
          <>
            {/* Header */}
