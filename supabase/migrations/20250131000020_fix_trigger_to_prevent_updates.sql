-- Migration: Fix trigger to prevent updating existing records
-- The trigger should only INSERT new records, never UPDATE existing ones
...
BEGIN
  ...
  IF NEW.status = 'completed' AND (OLD IS NULL OR OLD.status != 'completed') THEN
    ...
    -- If commission already exists, do nothing (prevent duplicate creation)
    ...
    -- If affiliate found, create commission record (ONLY INSERT, NEVER UPDATE)
