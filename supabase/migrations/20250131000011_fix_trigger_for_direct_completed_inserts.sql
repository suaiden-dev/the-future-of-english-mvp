-- Migration: Fix trigger to handle payments inserted directly with status 'completed'
-- The trigger should create a commission record when a payment is inserted with status 'completed'
-- even if it's not an UPDATE (OLD.status IS NULL on INSERT)
